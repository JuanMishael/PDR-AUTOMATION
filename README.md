...
- **Scenario builder** — assemble test steps from 30+ action types (click, fill, assert, screenshot, etc.) across Navigation, Interaction, Mouse, Assertions, Waits, Flow, Variables, and Util, with collapsible step cards for a clean overview
...
- **📌 Capture Value → `{{var.name}}`** — read something off the page mid-run (order number, generated ID, a computed total) and reuse it in any later step, condition, or assertion. Sources: element text, input value, an attribute, the URL, or a JS expression. It's the copy/paste a tester does by hand, minus the hand. Run-scoped on purpose — a value scraped off the UI is only true for *this* run, so it's never saved (unlike the API profile's persistent variable store)
- **⚡ Custom Code (advanced)** — an escape hatch for the one step the palette can't express: your own Playwright code with `page`, `context`, `expect`, and `vars` in scope, in a syntax-tinted editor. Write `vars.orderId = …` and `{{var.orderId}}` works everywhere downstream. It runs in the *runner* (unlike **Execute JS**, which is sandboxed to the page), so downloads, extra tabs and `context.request` are reachable. Keep it rare — steps that stay real step cards are the ones your non-technical testers can maintain
...
- **🎥 Run recording** — every run is recorded to a video, opened from the results via **Watch Recording**. Recording can't be started *after* a step fails, so it records passes and failures alike — attach the clip to a bug ticket instead of describing what you saw. Turn it off in **Settings**; the file is deleted with the run's history row (see History Retention)
...
- **Dashboard** — a card per profile showing scenario count, last-run time and the last run's "X/Y scenarios passed" breakdown, with **✎** to edit the profile itself (name, URL, browser, timeout) without going through New Profile
...
**Recently shipped** (was Phase 2): **📌 Capture Value + ⚡ Custom Code** (run-scoped `{{var.name}}`), **🔀 If/Else conditional blocks**, **🩹 self-healing selector chain** (auto-captured fallbacks, on clicks + fill/type/select), **🧪 per-run data-set picker** (independent per Run button) + representative-set resolution + ✎ inline data editing + unresolved-token warning, assert-mode recording, smart waits, Test Data Library + data-driven loops, step groups, drag-and-drop reordering, inline scenario rename, copy steps between scenarios, group-aware selection, **calm-playback network settle** (+ per-step `_noSettle` opt-out), **per-step network trace** in results, **🎥 run video recording** (+ artifact cleanup on retention), **parallel profile runs**, **Set Checkbox** action, a per-step **Max wait** on Assert Text, **replay parity for record/pick/test** (prereq Login chain + `{{token}}` resolution + first-set groups + `ERR_ABORTED`-tolerant navigate), **enforced history retention**, and **clean-shutdown browser reaping**.
