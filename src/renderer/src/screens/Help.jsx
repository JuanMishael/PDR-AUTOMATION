        ...
        <div style={{ maxWidth: 720 }}>
          ...
          <Section id="api" title={<>API Profiles <Beta /></>} refMap={refMap}
            ...
            <div className="card" style={{ marginBottom: 12 }}>
              ...
              <Define term="Preview resolved">The Body tab's <strong>⤓ Preview resolved</strong> shows the request with every token replaced by its real value for the selected row — exactly what Send transmits. Copy it to paste into SoapUI/Postman for comparison.</Define>
              ...
              <Define term="Extract">After a Send, the response <strong>Body</strong> shows as a 🌲 <strong>Tree</strong>. Click any value → name it → it's saved into a variable. <strong>Prefer clicking</strong> over hand-typing a path: the path is the <em>full</em> route to the value, not just the field name. For SOAP that means the whole envelope — <Code>Envelope.Body.genTokenResponse.genTokenResult</Code>, not <Code>genTokenResult</Code>. A shortened path matches nothing, so the variable silently stays empty. Clicking the value fills the correct path for you.</Define>
              ...
              <Define term="Export Postman">Saves the whole profile as a Postman Collection (v2.1) so you can run it in Postman. The headers the app adds for you (<Code>Content-Type</Code>, <Code>SOAPAction</Code>, the auth header) are written out explicitly, and a request bound to a Test Data collection becomes <strong>one Postman request per data row</strong> — so two rows ⇒ two requests. Test-data values are baked per row; the auth token stays a Postman collection variable so you can refresh it in one place.</Define>
