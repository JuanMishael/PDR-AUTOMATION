...
- **Reorder & arrange** — move steps and whole groups with ↑↓ (block-aware) or **drag-and-drop** (drop a step *into* a group to make it a member); **drag scenarios** to reorder them, with a per-scenario **⋯ menu** (rename / duplicate / delete) keeping the list clean; copy scenarios across profiles
